...
class GNMTModel(attention_model.AttentionModel):
  ...
  def _build_individual_encoder_layers(self, bi_encoder_outputs,
    ...
    with tf.compat.v1.variable_scope("rnn/multi_rnn_cell"):
      for i, cell in enumerate(uni_cell_lists):
        with tf.compat.v1.variable_scope("cell_%d" % i) as scope:
          encoder_inp, encoder_state = tf.compat.v1.nn.dynamic_rnn(
              cell,
              encoder_inp,
              dtype=dtype,
              sequence_length=self.iterator.source_sequence_length,
...
def gnmt_residual_fn(inputs, outputs):
  ...
  actual_inputs, _ = tf.nest.map_structure(
      split_input, inputs, outputs)
  def assert_shape_match(inp, out):
    inp.get_shape().assert_is_compatible_with(out.get_shape())
  tf.nest.assert_same_structure(actual_inputs, outputs)
