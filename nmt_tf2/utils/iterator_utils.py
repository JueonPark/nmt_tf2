...
def get_infer_iterator(src_dataset,
  ...
  src_dataset = src_dataset.map(lambda src: tf.compat.v1.string_split([src]).values)
  ...
  if use_char_encode:
    src_dataset = src_dataset.map(
        lambda src: (src,
                     tf.cast(
                         tf.size(input=src) / vocab_utils.DEFAULT_CHAR_MAXLEN, dtype=tf.int32)))
  else:
    src_dataset = src_dataset.map(lambda src: (src, tf.size(input=src)))
...
def get_iterator(src_dataset,
  ...
  src_tgt_dataset = src_tgt_dataset.filter(
      lambda src, tgt: tf.logical_and(tf.size(input=src) > 0, tf.size(input=tgt) > 0))
  ...
  if use_char_encode:
    src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in, tgt_out: (
            src, tgt_in, tgt_out,
            tf.cast(tf.size(input=src) / vocab_utils.DEFAULT_CHAR_MAXLEN, dtype=tf.int32),
            tf.size(input=tgt_in)),
        num_parallel_calls=num_parallel_calls)
  else:
    src_tgt_dataset = src_tgt_dataset.map(
        lambda src, tgt_in, tgt_out: (
