...
def _string_to_bytes(text, max_length):
  ...
  """
  byte_ids = tf.cast(tf.io.decode_raw(text, tf.uint8), dtype=tf.int32)
  byte_ids = byte_ids[:max_length - 2]
  padding = tf.fill([max_length - tf.shape(input=byte_ids)[0] - 2], PAD_CHAR_ID)
  byte_ids = tf.concat(
      [[BOW_CHAR_ID], byte_ids, [EOW_CHAR_ID], padding], axis=0)
  tf.compat.v1.logging.info(byte_ids)

  byte_ids = tf.reshape(byte_ids, [max_length])
  tf.compat.v1.logging.info(byte_ids.get_shape().as_list())
  return byte_ids + 1
...
def tokens_to_bytes(tokens):
  ...
  with tf.device("/cpu:0"):
    tf.compat.v1.assert_rank(tokens, 1)
    shape = tf.shape(input=tokens)
    tf.compat.v1.logging.info(tokens)
    ...
    as_bytes_flat = tf.map_fn(
        ...
        dtype=tf.int32,
        back_prop=False)
    tf.compat.v1.logging.info(as_bytes_flat)
...
def load_embed_txt(embed_file):
  ...
  is_first_line = True
  with codecs.getreader("utf-8")(tf.io.gfile.GFile(embed_file, "rb")) as f:
    for line in f:
