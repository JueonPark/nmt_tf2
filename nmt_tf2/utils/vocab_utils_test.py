...
class VocabUtilsTest(tf.test.TestCase):
  ...
  def testCheckVocab(self):
    # Create a vocab file
    vocab_dir = os.path.join(tf.compat.v1.test.get_temp_dir(), "vocab_dir")
    os.makedirs(vocab_dir)
    vocab_file = os.path.join(vocab_dir, "vocab_file")
    vocab = ["a", "b", "c"]
    with codecs.getwriter("utf-8")(tf.io.gfile.GFile(vocab_file, "wb")) as f:
      for word in vocab:
        f.write("%s\n" % word)

    # Call vocab_utils
    out_dir = os.path.join(tf.compat.v1.test.get_temp_dir(), "out_dir")
    os.makedirs(out_dir)
