...
def create_emb_for_encoder_and_decoder(share_vocab,
  ...
  if num_enc_partitions <= 1:
    ...
    # jobs.
    enc_partitioner = tf.compat.v1.fixed_size_partitioner(num_enc_partitions)
  ...
  if num_dec_partitions <= 1:
    ...
    # with lookup ops. This may cause embedding variables being placed on worker
    # jobs.
    dec_partitioner = tf.compat.v1.fixed_size_partitioner(num_dec_partitions)
...
def avg_checkpoints(model_dir, num_last_checkpoints, global_step,
  ...
  if not tf.io.gfile.exists(avg_model_dir):
    utils.print_out(
        "# Creating new directory %s for saving averaged checkpoints." %
        avg_model_dir)
    tf.io.gfile.makedirs(avg_model_dir)
