...
class ModelTest(tf.test.TestCase):
  ...
  def testNoAttentionNoResidualUniEncoder(self):
    ...
    with tf.Graph().as_default():
      with tf.compat.v1.Session(worker.target, config=self._get_session_config()) as sess:
        train_m = self._createTestTrainModel(model.Model, hparams, sess)

        m_vars = tf.compat.v1.trainable_variables()
        self._assertModelVariableNames(expected_var_names,
                                       [v.name for v in m_vars],
                                       'NoAttentionNoResidualUniEncoder')

        with tf.compat.v1.variable_scope('dynamic_seq2seq', reuse=True):
          last_enc_weight = tf.compat.v1.get_variable(
              'encoder/rnn/basic_lstm_cell/kernel')
          last_dec_weight = tf.compat.v1.get_variable('decoder/basic_lstm_cell/kernel')
    ...
    with tf.Graph().as_default():
      with tf.compat.v1.Session(worker.target, config=self._get_session_config()) as sess:
        ...
        with tf.compat.v1.variable_scope('dynamic_seq2seq', reuse=True):
          ...
          last_dec_weight = tf.compat.v1.get_variable(
    ...
    with tf.Graph().as_default():
      with tf.compat.v1.Session(worker.target, config=self._get_session_config()) as sess:
        ...
        with tf.compat.v1.variable_scope('dynamic_seq2seq', reuse=True):
          ...
          att_layer_weight = tf.compat.v1.get_variable(
    ...
    with tf.Graph().as_default():
      with tf.compat.v1.Session(worker.target, config=self._get_session_config()) as sess:
        ...
        with tf.compat.v1.variable_scope('dynamic_seq2seq', reuse=True):
          ...
          mem_layer_weight = tf.compat.v1.get_variable('decoder/memory_layer/kernel')
        self._assertTrainStepsLoss(train_m, sess,
                                   'UniEncoderStandardAttentionArchitecture')
        self._assertModelVariable(
            last_enc_weight, sess,
