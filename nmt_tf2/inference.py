...
def _decode_inference_indices(model, sess, output_infer,
  ...
  with codecs.getwriter("utf-8")(
      tf.io.gfile.GFile(output_infer, mode="wb")) as trans_f:
    ...
    for decode_id in inference_indices:
      ...
      if infer_summary is not None:  # Attention models
        ...
        image_summ = tf.compat.v1.Summary()
        image_summ.ParseFromString(infer_summary)
        with tf.io.gfile.GFile(image_file, mode="w") as img_f:
...
def multi_worker_inference(sess,
  ...
  with infer_model.graph.as_default():
    ...
    tf.io.gfile.rename(output_infer, output_infer_done, overwrite=True)
    ...
    with codecs.getwriter("utf-8")(
        tf.io.gfile.GFile(final_output_infer, mode="wb")) as final_f:
      for worker_id in range(num_workers):
        worker_infer_done = "%s_done_%d" % (inference_output_file, worker_id)
        while not tf.io.gfile.exists(worker_infer_done):
          utils.print_out("  waiting job %d to complete." % worker_id)
          time.sleep(10)

        with codecs.getreader("utf-8")(
            tf.io.gfile.GFile(worker_infer_done, mode="rb")) as f:
          for translation in f:
            final_f.write("%s" % translation)

      for worker_id in range(num_workers):
        worker_infer_done = "%s_done_%d" % (inference_output_file, worker_id)
        tf.io.gfile.remove(worker_infer_done)
