...
class BaseModel(object):
  ...
  def __init__(self,
    ...
    self.saver = tf.compat.v1.train.Saver(
        tf.compat.v1.global_variables(), max_to_keep=hparams.num_keep_ckpts)
  ...
  def _set_params_initializer(self,
    ...
    # Batch size
    self.batch_size = tf.size(input=self.iterator.source_sequence_length)
  ...
  def _get_learning_rate_warmup(self, hparams):
    ...
    if warmup_scheme == "t2t":
      ...
      inv_decay = warmup_factor**(
          tf.cast(warmup_steps - self.global_step, dtype=tf.float32))
    else:
      raise ValueError("Unknown warmup scheme %s" % warmup_scheme)

    return tf.cond(
        pred=self.global_step < hparams.warmup_steps,
        true_fn=lambda: inv_decay * self.learning_rate,
  ...
  def _get_learning_rate_decay(self, hparams):
    ...
    return tf.cond(
        pred=self.global_step < start_decay_step,
  ...
  def _get_infer_maximum_iterations(self, hparams, source_sequence_length):
    ...
    if hparams.tgt_max_len_infer:
      ...
      max_encoder_length = tf.reduce_max(input_tensor=source_sequence_length)
  ...
  def _softmax_cross_entropy_loss(
    ...
    if self.num_sampled_softmax > 0:
      ...
      crossent = tf.nn.sampled_softmax_loss(
          weights=tf.transpose(a=self.output_layer.kernel),
  ...
  def _compute_loss(self, logits, decoder_cell_outputs):
    ...
    loss = tf.reduce_sum(
        input_tensor=crossent * target_weights) / tf.cast(self.batch_size, dtype=tf.float32)
    return loss
  ...
  def build_encoder_states(self, include_embeddings=False):
    ...
    if self.time_major:
      stack_state_list = tf.transpose(a=stack_state_list, perm=[1, 0, 2, 3])
...
class Model(BaseModel):
  ...
  def _build_encoder_from_sequence(self, hparams, sequence, sequence_length):
    ...
    if self.time_major:
      sequence = tf.transpose(a=sequence)
  ...
  def _build_bidirectional_rnn(self, inputs, sequence_length,
    ...
    bi_outputs, bi_state = tf.compat.v1.nn.bidirectional_dynamic_rnn(
        fw_cell,
        bw_cell,
        inputs,
        dtype=dtype,
